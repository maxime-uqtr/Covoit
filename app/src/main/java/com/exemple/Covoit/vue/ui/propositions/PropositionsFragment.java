...
public class PropositionsFragment extends Fragment implements OnListClickListener, SwipeRefreshLayout.OnRefreshListener {

    private static CovoiturageService apiInterface;
    private CovoiturageBd bd;

    private TrajetsViewModel propositionsViewModel;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater,
        ...
        propositionsViewModel =
                ViewModelProviders.of(this).get(TrajetsViewModel.class);
        View view = inflater.inflate(R.layout.fragment_propositions, container, false);
        apiInterface = ApiClient.getApiClient().create(CovoiturageService.class);

        swipeContainer = view.findViewById(R.id.propositions_swipeContainer);
        Utilisateur user = UtilisateurActuel.getUtilisateur();
        propositionsViewModel.getText().observe(getViewLifecycleOwner(), new Observer<String>() {
            @Override
        ...
        rv = view.findViewById(R.id.propositions_recyclerView);
        texte = view.findViewById(R.id.propositions_texte);
        ...
        if(user.isConducteur()) {
            ...
            texte.setText(R.string.tvConducteur);
    ...
    public void onResume() {
        ...
        if(u.isPassager()) {
            adaptateur.clear();
            ControleurPropositions.updateOffers(adaptateur, apiInterface, getActivity().getApplicationContext()); //On actualise les trajets
        }
    }
    ...
    public void onRefresh() {
        ...
        if(u.isPassager()) {
            adaptateur.clear();
            ControleurPropositions.updateOffers(adaptateur, apiInterface, getActivity().getApplicationContext());
