...
public class ControleurPropositions {
    public static void updateOffers(ListAdapteur adaptateur, CovoiturageService apiInterface, Context context) {
        adaptateur.clear(); //On vide l'adaptateur
        long sessionU = UtilisateurActuel.getUtilisateur().getId(); //Id de l'utilisateur
        Call<List<Covoiturage>> call = apiInterface.getConducteur(sessionU);
    ...
    public static void proposer(Covoiturage c, CovoiturageService apiInterface, Context context) {
        Utilisateur user  = UtilisateurActuel.getUtilisateur(); //Id de l'utilisateur
        CovoiturageBd.getInstance(context).getUtilisateurDao().insert(user);
        Call<ResponseBody> call = apiInterface.insert(c.getDate(), c.getVilleDep(), c.getVilleArr(), c.getPrix(), c.getNbPassager(), c.getConducteurId());
        call.enqueue(new Callback<ResponseBody>() {
            @Override
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                CovoiturageBd.getInstance(context).getCovoiturageDao().insert(c);
            }
